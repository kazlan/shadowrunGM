...
if (!mapActionHtml.includes('node-map__jack-out--safe') || !mapActionHtml.includes('aria-label="Jack out: ruta limpia"') || !themeSource.includes('.node-map__jack-out--safe') || !themeSource.includes('jackOutSafeTrace')) throw new Error('Jack out should use the primary animated style on entry and exit nodes');
const unsafeJackOutMapHtml = renderNodeMap(projectSystemForRun(iceSystem, movedIntoIce), movedIntoIce);
if (unsafeJackOutMapHtml.includes('node-map__jack-out--safe') || unsafeJackOutMapHtml.includes('Jack out: ruta limpia')) throw new Error('Jack out should not show the clean-route style away from entry or exit nodes');
