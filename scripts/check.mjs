...
if (!mainSource.includes('triggerMapElasticMotion') || !mainSource.includes('MAP_ELASTIC_MOTION_MS') || !mainSource.includes('MAP_BIG_BANG_MOTION_MS') || !mainSource.includes('applyMapExpansionMotion') || !mainSource.includes('updateElasticRoutes') || !mainSource.includes('prefersReducedMotion')) throw new Error('Node map should drive deterministic expansion on opening and elastic motion for gestures');
if (!nodeMapSource.includes('data-base-x') || !nodeMapSource.includes('data-motion-seed') || !nodeMapSource.includes('data-edge-from') || !nodeMapSource.includes(' Q ') || `${mainSource}\n${nodeMapSource}`.includes('d3-force')) throw new Error('Node map should expose deterministic animation metadata and curved routes without a force engine');
...
if (!overpassSource.includes('const ALLOWED_RADIUS = [250, 2000]') || !overpassSource.includes('nwr["amenity"]["name"]') || !overpassSource.includes('nwr["shop"]["name"]') || !overpassSource.includes('nwr["leisure"]["name"]') || !mainSource.includes('EXPANDED_SCAN_RADIUS')) throw new Error('Local scanner should broaden OSM target searches before using local fallback nodes');
...
if (!mainSource.includes('const EXPANDED_SCAN_RADIUS = 2000') || mainSource.includes('Sin objetivos reales') || mainSource.includes('objetivos demo en') || mainSource.includes('tras ampliar a') || mainSource.includes('(${radius}m)')) throw new Error('Scanner should expand sparse searches silently without exposing radius or provider fallback diagnostics');
...
if (!deckOverlayHtml.includes('Área 0') || deckOverlayHtml.includes('Banco de trabajo') || deckOverlayHtml.includes('overlay-close') || !deckOverlayHtml.includes('deck-workbench__balance') || !deckOverlayHtml.includes('<b>¤</b>') || !deckOverlayHtml.includes('Scanner de objetivos') || deckOverlayHtml.includes('deck-workbench__radar') || deckOverlayHtml.includes('deck-workbench__scanner"') || !deckOverlayHtml.includes('data-action="toggleScanner"') || !deckOverlayHtml.includes('deck-workbench__message') || !deckOverlayHtml.includes('deck-workbench__section--hardware')) throw new Error('Area 0 overlay should render balance, standalone scanner CTA, compact upgrade message, and no close button');
...
if (!themeSource.includes('.deck-workbench {') || !themeSource.includes('height: 100dvh') || !themeSource.includes('width: 100%') || !themeSource.includes('overflow-y: auto') || !themeSource.includes('gap: 20px') || !themeSource.includes('.deck-workbench__scanner-button:hover') || !themeSource.includes('.deck-workbench__section--hardware .deck-upgrade-list') || !themeSource.includes('grid-template-columns: repeat(5, minmax(0, 1fr))')) throw new Error('Workbench overlay should cover the viewport and keep loaded software in one compact row');
if (mainSource.includes('transferidos a la cuenta')) throw new Error('Area 0 should not repeat the post-run transfer report');
...
if (!mapActionHtml.includes('data-base-x=') || !mapActionHtml.includes('data-base-y=') || !mapActionHtml.includes('data-motion-seed=') || !mapActionHtml.includes('data-edge-from=') || !mapActionHtml.includes('data-edge-to=') || !mapActionHtml.includes(' Q ')) throw new Error('Active node maps should render curved elastic-ready routes and node motion anchors');
if (!mapActionHtml.includes('node-map__actions') || !mapActionHtml.includes('data-action="openHelp"') || !mapActionHtml.includes('node-map__help-button') || !mapActionHtml.includes('data-action="toggleSettings"') || !mapActionHtml.includes('node-map__avatar-settings') || mapActionHtml.includes('data-action="jackOut"') || !themeSource.includes('.node-map__help-button')) throw new Error('Node map should expose log/help/settings actions and keep jack-out out of the top-right stack');
...
const safeJackOutDockHtml = renderProgramDock({ ...jackOutRun, selectedProgram: 'scan', disabledPrograms: [] }, false, null, upgradedDeckResult.profile, { safe: true, label: 'Jack out: ruta limpia' });
if (!safeJackOutDockHtml.includes('program-dock--with-jackout') || !safeJackOutDockHtml.includes('program-jackout') || !safeJackOutDockHtml.includes('is-safe-jackout') || !safeJackOutDockHtml.includes('data-action="jackOut"') || !safeJackOutDockHtml.includes('aria-label="Jack out: ruta limpia"') || !safeJackOutDockHtml.includes('<span>Jack out</span>') || !themeSource.includes('.program-dock .program-jackout.is-safe-jackout') || !themeSource.includes('transform: translate(-50%, -50%) rotate(90deg)') || !themeSource.includes('justify-self: end') || !themeSource.includes('width: clamp(34px, 4vw, 42px)') || !themeSource.includes('text-align: center') || !themeSource.includes('jackOutSafeTrace')) throw new Error('Jack out should live in the program dock with primary animated style, right alignment, narrow width, and centered rotated text on entry and exit nodes');
const unsafeJackOutDockHtml = renderProgramDock({ ...movedIntoIce, selectedProgram: 'scan', disabledPrograms: [] }, false, null, upgradedDeckResult.profile, { safe: false, label: 'Jack out' });
if (!unsafeJackOutDockHtml.includes('program-jackout') || unsafeJackOutDockHtml.includes('is-safe-jackout') || unsafeJackOutDockHtml.includes('Jack out: ruta limpia')) throw new Error('Jack out should not show the clean-route style away from entry or exit nodes');
...
if (finishedActionMapHtml.includes('node-map__actions') || finishedActionMapHtml.includes('node-map__jack-out') || finishedActionMapHtml.includes('data-action="jackOut"')) throw new Error('Finished maps should remove the active top-right action stack');
const finishedProgramDockHtml = renderProgramDock({ ...jackOutRun, selectedProgram: 'scan', disabledPrograms: [] }, true, null, upgradedDeckResult.profile, { safe: true, label: 'Jack out: ruta limpia' });
if (!finishedProgramDockHtml.includes('program-dock--inactive') || finishedProgramDockHtml.includes('program-jackout')) throw new Error('Finished runs should fade and disable program controls without rendering jack-out');
...
const runHelpHtml = renderHelpOverlay(true, 'run');
if (!renderHelpOverlay(true, 'deck').includes('help-tabs') || !runHelpHtml.includes('help-section--run') || !themeSource.includes('text-align: left') || themeSource.includes('text-align: justify') || !themeSource.includes('overflow-wrap: normal') || !themeSource.includes('hyphens: none') || !themeSource.includes('repeat(auto-fit, minmax(min(100%, 180px), 1fr))') || !themeSource.includes('.help-section--run .help-grid') || !themeSource.includes('grid-template-columns: repeat(3, minmax(0, 1fr))')) throw new Error('Help overlay should render compact tab navigation with left-aligned, non-clipped text and three-column RUN grids in its own dialog');
